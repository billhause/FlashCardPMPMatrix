...
//        .foregroundColor(Color.blue)
    ...
    func cardTapHandler(row: Int16, col: Int16) {
        ...
        flashCardEntity.text = "☃️ Snow Man row: \(row), col: \(col)"
...
//     var flashCardEntities: FetchedResults<FlashCardEntity>
    ...
    @FetchRequest(
        sortDescriptors: [NSSortDescriptor(keyPath: \FlashCardEntity.row, ascending: true), NSSortDescriptor(keyPath: \FlashCardEntity.column, ascending: true)],
        animation: .default)
    ...
    ForEach(flashCardEntities, id: \.self) { flashCardEntity in
            if (flashCardEntity.row == row) && (flashCardEntity.column == column) {
                ZStack {
                    ...
                    Text("\(flashCardEntities[Int(column + row*6)].text!)")
               }
           }
    }
...
/*
        // vv UPDATE ALL CELLS when Key Cell is touched vv
        ZStack {
            RoundedRectangle(cornerRadius: 5.0).fill(fillColor)
            RoundedRectangle(cornerRadius: 5.0).stroke(lineWidth:1)
            Text("\(flashCardEntities.first!.text!)")
         }
  */
        
/*
            // vv USING STATIC METHOD TO LOAD REQUESTED CELL vvv
            ZStack {
                RoundedRectangle(cornerRadius: 5.0).fill(fillColor)
                RoundedRectangle(cornerRadius: 5.0).stroke(lineWidth:1)
