            ...
            ForEach(0..<FlashCardViewModel.columnCount) { colNumber in
                ...
                Group { // COLOR Based on Row, Column
                    ...
                    if (colNumber == 0) && (rowNumber == 0) { // ORIGIN CELL
                        CardView(row: rowNumber, column: Int16(colNumber), fillColor: colorScheme == .light ? originCellColorLight : originCellColorDark)
                        ...
                        CardView(row: rowNumber, column: Int16(colNumber), fillColor: colorScheme == .light ? rowHeaderColorLight : rowHeaderColorDark)
                            .onTapGesture(count:2) {
                                cardDoubleTapHandler(row: rowNumber, col: Int16(colNumber))
                            }
                            .onTapGesture(count:1) {
                        ...
                        CardView(row: rowNumber, column: Int16(colNumber), fillColor: colorScheme == .light ? columnHeaderColorLight : columnHeaderColorDark)
                            .onTapGesture(count:2) {
                                cardDoubleTapHandler(row: rowNumber, col: Int16(colNumber))
                            }
                            .onTapGesture(count:1) {
                        ...
                        CardView(row: rowNumber, column: Int16(colNumber), fillColor: colorScheme == .light ? tableCellColorLight : tableCellColorDark)
                            .onTapGesture(count:2) {
                                cardDoubleTapHandler(row: rowNumber, col: Int16(colNumber))
                            }
                            .onTapGesture(count:1) {
...
//        flashCardEntity.objectWillChange.send() // Cause Views to update Stanford Lesson 12 at 52:20
    ...
    // Light Color Scheme
    let columnHeaderColorLight: Color = Color.init(red: 0.7, green: 0.9, blue: 1.0)
    let rowHeaderColorLight:    Color = Color.init(red: 0.9, green: 1.0, blue: 0.9)
    let originCellColorLight:   Color = Color.init(red: 1.0, green: 1.0, blue: 1.0)
    let tableCellColorLight:    Color = Color.init(red: 0.9, green: 0.9, blue: 0.9)
    
    // Dark Mode Color Scheme
    let columnHeaderColorDark: Color = Color.init(red: 0.2, green: 0.4, blue: 0.8)
    let rowHeaderColorDark:    Color = Color.init(red: 0.22, green: 0.4, blue: 0.22)
    let originCellColorDark:   Color = Color.init(red: 0.1, green: 0.1, blue: 0.1)
    let tableCellColorDark:    Color = Color.init(red: 0.22, green: 0.22, blue: 0.22)
...
struct CardView: View {
    ...
    var body: some View {
        var questionMarkColor = Color.init(red: 0.2, green: 0.4, blue: 1.0)
        if colorScheme == .dark {
            questionMarkColor = Color.init(red: 0.8, green: 0.9, blue: 1.0)
        }
...
// use default text color for dark mode           .foregroundColor(forgroundColor)
                    }.transition(.scale)
